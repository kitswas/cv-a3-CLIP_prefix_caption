...
def save_pickle(data, out_path: str, recover_index: int | None = None):
        ...
        if num_threads == 1:
            ...
            for i in range(num_threads):
                threads.append(
                    threading.Thread(
                        target=thread,
                        args=(groups[i], i, progress, lock, suffix, conceptual_root),
                    )
                )
            for i in range(num_threads):
                threads[i].start()
...
def create_clip_embeddings(conceptual_root: str, clip_model_type: str):
    ...
    for suffix in ("val", "train"):
        ...
        dl = DataLoader(
            ds, batch_size=200, shuffle=False, num_workers=8, drop_last=False
        )
        progress = tqdm(total=len(dl))
        counter = 0
        clip_model_name = clip_model_type.replace("/", "_")
        out_data_path = (
            f"{conceptual_root}/conceptual_clip_{clip_model_name}_{suffix}.pkl"
        )
        recover_index = 0
        for i, data in enumerate(dl):
            ...
            image_names = [
                image_name for j, image_name in enumerate(image_names) if is_valid[j]
            ]
            all_captions.extend(
                [
                    {
                        "caption": add_period(caption),
                        "clip_embedding": counter + j,
                        "image_id": image_name,
                    }
                    for j, (caption, image_name) in enumerate(
                        zip(captions, image_names)
                    )
                ]
            )
            progress.update()
